  ...
  return (
    <nav className="border-b border-zinc-900 bg-zinc-950/80 backdrop-blur-xl sticky top-0 z-50 p-4 font-sans selection:bg-emerald-500/30">
      <div className="max-w-7xl mx-auto flex justify-between items-center gap-6">
        ...
        {/* --- BRAND LOGO --- */}
        <Link href="/" onClick={() => setIsMenuOpen(false)} className="hover:opacity-90 transition-opacity">
          <Logo />
        </Link>
        ...
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className="md:hidden flex items-center gap-2 bg-zinc-900 px-4 py-2 rounded-full border border-zinc-800 active:scale-95 transition-transform"
          >
            <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></span>
            <span className="text-emerald-500 font-black text-[10px] uppercase tracking-widest italic">
      ...
      {/* --- ✅ MOBILE OVERLAY MENU --- */}
      {isMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-zinc-950 border-b border-zinc-900 p-8 space-y-8 animate-in slide-in-from-top duration-300 shadow-2xl">
          <div className="flex flex-col gap-6 text-center">
            <Link 
              href="/dashboard" 
              onClick={() => setIsMenuOpen(false)}
              className="text-lg font-black uppercase tracking-[0.2em] text-white hover:text-emerald-500"
            >
              My Dashboard
            </Link>
            <Link 
            ...
            {isAdmin && (
              <Link 
                href="/admin" 
                onClick={() => setIsMenuOpen(false)}
                className="text-lg font-black uppercase tracking-[0.2em] text-red-500 bg-red-500/10 py-3 rounded-2xl border border-red-500/20"
              >
                Admin Control
              </Link>
            )}
            ...
            <div className="pt-6 border-t border-zinc-900 flex flex-col gap-4">
              {user ? (
                <button 
                  onClick={handleLogout}
                  className="w-full py-4 bg-zinc-900 text-zinc-400 font-black uppercase tracking-widest rounded-2xl border border-zinc-800"
                >
                  Logout Mission
                </button>
              ) : (
                <>
                  <Link 
                    href="/login" 
                    onClick={() => setIsMenuOpen(false)}
                    className="w-full py-4 bg-zinc-900 text-white font-black uppercase tracking-widest rounded-2xl border border-zinc-800"
                  >
                    Hero Login
                  </Link>
                  <Link 
                    href="/signup" 
                    onClick={() => setIsMenuOpen(false)}
                    className="w-full py-4 bg-emerald-600 text-white font-black uppercase tracking-widest rounded-2xl shadow-lg italic"
                  >
                    Join Now
                  </Link>
                </>
